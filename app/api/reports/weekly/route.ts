  ...
  const previousAvg =
    previousSessions.length > 0
      ? previousSessions.reduce(
          (acc: number, s: any) => acc + s.focusPercentage,
          0
...
async function calculateProductivityTrend(db: any, userId: string) {
  ...
  const weeklyAverages = Object.values(weeks).map(
    (weekSessions: any) =>
      weekSessions.reduce((acc: number, s: any) => acc + s.focusPercentage, 0) /
      weekSessions.length
  );

  // Simple trend: positive if last week > first week
  if (weeklyAverages.length >= 2) {
    return weeklyAverages[weeklyAverages.length - 1] > weeklyAverages[0]
      ? "improving"
      : "declining";
  }
...
function getRecommendations(sessions: any[]) {
  ...
  if (avgFocus < 70) {
    recommendations.push(
      "Consider shorter focus sessions to maintain concentration"
    );
  }
  ...
  if (recommendations.length === 0) {
    recommendations.push(
      "Great work! Keep maintaining your excellent focus habits"
    );
  }
